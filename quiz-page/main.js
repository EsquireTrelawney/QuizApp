const QUIZ_CATEGORIES = [
  {category: 'example', title:'Лунтик и его друзья'},
...
class UI {
  ...
  *initQuizGenerator() {
  ...
  showIsCorrect(currentQuizIndex, isCorrect = true) {
    if (currentQuizIndex < this.indicators.length) {
      if (isCorrect) {
      ...
      console.error(`Ошибка: Не удалось найти индикатор для индекса ${currentQuizIndex}`);
    }
  }
  ...
  showResult(numberOfCorrectAnswers, totalQuestions, cardsWithAnswers) {
    this.hideElement(this.quiz);
    this.resultCard.classList.remove('hidden');

    this.result.innerHTML = `${numberOfCorrectAnswers} / ${totalQuestions}`;
...
class Controller {
  ...
  async submitAnswer(byUser) {
    ...
    // Добавка проверки на окончание списка вопросов
    if (result) {
      this.game.incrementCurrentQuizIndex();
      //if (this.game.getCurrentQuizIndex() < this.game.numberOfQuizzes) {
        this.startTimer();
        this.ui.showSubmitButton();
      /*} else {*/
        // если достигнут конец вопросов, показываем результаты
        //this.ui.showResult(
...
class QuizService {
  ...
  shuffleQuizzes(quizzes) {
    ...
    for (let i = 0; i < this.game.numberOfQuizzes; i++) {
...
function submitQuizResults() {
  // Будущий код для сбора рез-тов викторины
